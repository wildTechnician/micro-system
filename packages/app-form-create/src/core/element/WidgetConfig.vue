<template>
  <el-form v-if="data" :key="data.key" label-position="top">
    ...
    <el-form-item
      v-if="
        hasKey('defaultValue') &&
        (data.type === 'input' || data.type === 'password' || data.type === 'textarea' || data.type === 'text' || data.type === 'rate' || data.type === 'switch' || data.type === 'slider')
      "
      label="默认内容"
    >
      <el-input v-if="data.type === 'input' || data.type === 'password' || data.type === 'text'" v-model="data.options.defaultValue" />
      <el-input v-if="data.type === 'textarea'" v-model="data.options.defaultValue" type="textarea" />
      <el-rate v-if="data.type === 'rate'" v-model="data.options.defaultValue" :max="data.options.max" :allow-half="data.options.allowHalf" />
      ...
      <template v-else>
        <template v-if="data.type === 'radio' || (data.type === 'select' && !data.options.multiple)">
          <el-radio-group v-model="data.options.defaultValue" style="margin-top: 8px">
            <Draggable tag="ul" item-key="index" ghost-class="ghost" handle=".drag-item" :group="{ name: 'options' }" :list="data.options.options">
              <template #item="{ element, index }">
                <div style="display: flex; align-items: center; margin-bottom: 5px">
                  ...
                  <SvgIcon style="margin: 0 5px; cursor: move" icon-class="item" class-name="drag-item" />
        ...
        <div style="margin-top: 5px">
          <el-button link @click="handleInsertOption">添加选项</el-button>
        </div>
      </template>
    </el-form-item>
    ...
    <template v-if="data.type === 'time'">
      <el-form-item label="默认值">
        <el-time-picker v-model="data.options.defaultValue" style="width: 100%" :format="data.options.format" :placeholder="data.options.placeholder" />
      </el-form-item>
    </template>

    <template v-if="data.type === 'date'">
      <el-form-item label="默认值">
        <el-date-picker v-model="data.options.defaultValue" style="width: 100%" :format="data.options.format" :placeholder="data.options.placeholder" />
      </el-form-item>
    </template>
    ...
    <template v-if="data.type === 'grid'">
      ...
      <el-form-item label="列配置项">
        <Draggable tag="ul" item-key="index" ghost-class="ghost" handle=".drag-item" :group="{ name: 'options' }" :list="data.columns">
          <template #item="{ element, index }">
            <li style="margin-bottom: 5px">
              <SvgIcon icon-class="item" class-name="drag-item" />
        ...
        <div>
          <el-button link @click="handleInsertColumn"> 添加列 </el-button>
        </div>
      </el-form-item>
    ...
    <template v-if="data.type !== 'grid'">
      <el-form-item v-if="hasKey('rules') || hasKey('readonly') || hasKey('disabled') || hasKey('allowClear')" label="操作属性">
