<template>
  <div class="widget-form-container">
    ...
    >
      <Draggable class="widget-form-list" item-key="key" ghost-class="ghost" handle=".drag-widget" :animation="200" :group="{ name: 'people' }" :list="widgetForm.list" @add="handleMoveAdd">
            ...
            <template v-if="element.type === 'grid'">
              ...
              >
                ...
                <div v-if="widgetFormSelect?.key === element.key" class="widget-view-action widget-col-action">
                  <el-icon color="#fff" @click.stop="handleDeleteClick(index, widgetForm.list)">
...
import { Delete as DeleteIcon, Rank } from '@packages/utils-icon/src/elementPlus';
      ...
      if (row.columns[index].list[newIndex].type === 'radio' || row.columns[index].list[newIndex].type === 'checkbox' || row.columns[index].list[newIndex].type === 'select') {
        row.columns[index].list[newIndex] = {
          ...
          options: {
            ...
            options: row.columns[index].list[newIndex].options.options.map((item: any) => ({ ...item })),
          },
        };
      }
